...
export default function EthercheckGraphPage() {
        ...
        fetch(url)
            ...
            .then((data) => {
                ...
                if (loaded.length > 0) {
                    ...
                    setRooms(cleanRooms);
                    if (cleanRooms[0]) setSelectedRooms([cleanRooms[0]]);
                }
            })
            .catch((err) => {
                console.error(err);
    ...
    async function fetchGraph() {
        ...
        try {
            ...
            const cleanDate = (d) => d.replace("T", " ");
            params.append("date_start", cleanDate(dateStart));
            params.append("date_end", cleanDate(dateEnd));

            selectedRooms.forEach(r => params.append("rooms", r));

            console.log("Запрос GET:", `/api/graph?${params.toString()}`);
                ...
                <div className="w-full h-[500px] bg-[#0B1120] border border-slate-800 rounded-xl p-4 relative">
                    {!graphResult && !loadingGraph && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 pointer-events-none">
                            <svg className="w-12 h-12 mb-3 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path></svg>
